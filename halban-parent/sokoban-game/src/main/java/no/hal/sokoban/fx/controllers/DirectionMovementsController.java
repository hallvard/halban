  ...
  public Region getContent() {
    ...
    if (includeStandardButtons) {
      gridPane.getChildren().addAll(List.of(
          leftButton = createMovementButton(createFontIcon("mdi2t-triangle:" + iconSize, 270), Direction.LEFT, false,
              posMin, this.centerPos),
          rightButton = createMovementButton(createFontIcon("mdi2t-triangle:" + iconSize, 90), Direction.RIGHT, false,
              posMax, this.centerPos),
    ...
    if (includeMoveAlongButtons) {
      gridPane.getChildren().addAll(List.of(
          leftAlongButton = createMovementButton(createFontIcon("mdi2f-fast-forward:" + iconSize, 180), Direction.LEFT,
              true, posMin, this.centerPos),
          rightAlongButton = createMovementButton(createFontIcon("mdi2f-fast-forward:" + iconSize, 0), Direction.RIGHT,
              true, posMax, this.centerPos),
          upAlongButton = createMovementButton(createFontIcon("mdi2f-fast-forward:" + iconSize, 270), Direction.UP,
              true, this.centerPos, posMin),
          downAlongButton = createMovementButton(createFontIcon("mdi2f-fast-forward:" + iconSize, 90), Direction.DOWN,
              true, this.centerPos, posMax)));
    }
    Map<KeyCodeCombination, Button> shortcuts = new HashMap<>();
    if (leftButton != null)
      shortcuts.put(new KeyCodeCombination(KeyCode.LEFT), leftButton);
    if (rightButton != null)
      shortcuts.put(new KeyCodeCombination(KeyCode.RIGHT), rightButton);
    if (upButton != null)
      shortcuts.put(new KeyCodeCombination(KeyCode.UP), upButton);
    if (downButton != null)
      shortcuts.put(new KeyCodeCombination(KeyCode.DOWN), downButton);
    if (leftAlongButton != null)
      shortcuts.put(new KeyCodeCombination(KeyCode.LEFT, KeyCodeCombination.SHIFT_DOWN), leftAlongButton);
    if (rightAlongButton != null)
      shortcuts.put(new KeyCodeCombination(KeyCode.RIGHT, KeyCodeCombination.SHIFT_DOWN), rightAlongButton);
    if (upAlongButton != null)
      shortcuts.put(new KeyCodeCombination(KeyCode.UP, KeyCodeCombination.SHIFT_DOWN), upAlongButton);
    if (downAlongButton != null)
      shortcuts.put(new KeyCodeCombination(KeyCode.DOWN, KeyCodeCombination.SHIFT_DOWN), downAlongButton);
