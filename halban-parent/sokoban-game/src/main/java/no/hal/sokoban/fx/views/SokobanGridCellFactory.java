...
class SokobanGridCellFactory extends
    ...
    public SokobanGridCellFactory(Configuration config) {
        super(cellKind -> SokobanGridCellFactory.class.getResource("/no/hal/sokoban/fx/images/default/%s.png".formatted(
				switch (cellKind) {
					case WALL 			-> config.getString("sokoban.grid.wall", "wall16x16");
					case EMPTY 			-> config.getString("sokoban.grid.empty", "empty16x16");
					case TARGET 		-> config.getString("sokoban.grid.target", "target16x16");
					case EMPTY_PLAYER 	-> config.getString("sokoban.grid.player", "player16x16");
					case TARGET_PLAYER 	-> config.getString("sokoban.grid.playerOnTarget", "player_on_target16x16");
					case EMPTY_BOX 		-> config.getString("sokoban.grid.box", "box16x16");
					case TARGET_BOX 	-> config.getString("sokoban.grid.boxOnTarget", "box_on_target16x16");
				})).toExternalForm());
			}
/* 
